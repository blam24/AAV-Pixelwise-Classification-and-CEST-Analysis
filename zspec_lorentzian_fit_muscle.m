function F = zspec_lorentzian_fit_muscle(x_corrected, zspec, method)
...
n_seg = size(zspec,2);
n_interp = 1000;
cutoffs = [-3, -1.4, 1.4, 3]; % exclude points between -4ppm and -1.4 ppm, and between 1.4ppm and 4ppm (CEST metabolites region)
...
if strcmp(method,'2stepLD')
    ...
    %      [M0]  |Water~~~~~~~~~|   |MT~~~~~~~~~~~|
    P0 = [ 1    0.8    1.8     0     0.35  40   -1]; % starting point
    lb = [ 1    0.02   0.3     0     0.0   30   -2.5]; % lower bound
    ub = [ 1    1.0    10      0     0.6   60    0]; % upper bound
